...
def objective(trial):
    params = {
        "n_estimators": trial.suggest_int("n_estimators_gb", 10, 1000),
        "learning_rate": trial.suggest_float("learning_rate_gb", 1e-2, 0.1, log=True),
        "max_depth": trial.suggest_int("max_depth_gb", 2, 50),
        "objective": "multi:softmax",
        "device": "gpu",
    }

    # Initialize the model with suggested parameters
    model = XGBClassifier(**params, categorical_feature=categorical_var)

    # Train the model
    model.fit(processed_X_train, y_train)

    # Predict on validation set
    val_predictions = model.predict(processed_X_val)
    val_f1 = f1_score(y_val, val_predictions, average="weighted")

    # Predict on training set for overfitting diagnostics
    train_predictions = model.predict(processed_X_train)
    ...
    return val_f1  # Optuna optimizes based on validation F1 score
...
if __name__ == "__main__":
    ...
    print(f"  Train F1 Score: {best_trial.user_attrs['train_f1']}")
    print(f"  Validation F1 Score: {best_trial.user_attrs['val_f1']}")
